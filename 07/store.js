  ...
  addNewItem: function (item) {
    ...
    if(item.name){
        if(todos.some(x=> x.name === item.name)){
          let index = todos.findIndex(x=> x.name === item.name);
            todos[index].category= item.category;
            todos[index].shops = item.shops;
        }else{
    todos.push(item);
        }
    }else{
        alert("Produkt musi posiadać nazwę");
    }
...
console.log(filter.category);
    ...
    todos = todos
      ...
      .filter(x => !filter.category || x.category == filter.category) // category

       // shops; shops jest tablicą; zastosuj warunek lub ||
    .filter(x=> filter.shops.length===0 || x.shops.some(shop=> filter.shops.indexOf(shop)!==-1) );
